...
struct Varchar {
   ...
   Varchar(const char* str, size_t l)
   {
      assert(l <= maxLength);
      length = l;
      memcpy(data, str, l);
...
inline unsigned fold(uint8_t* writer, const Integer& x)
{
   *reinterpret_cast<uint32_t*>(writer) = __builtin_bswap32(x ^ (1ul << 31));
   return sizeof(x);
}

inline unsigned fold(uint8_t* writer, const Timestamp& x)
{
...
inline unsigned unfold(const uint8_t* input, Integer& x)
{
   x = __builtin_bswap32(*reinterpret_cast<const uint32_t*>(input)) ^ (1ul << 31);
   return sizeof(x);
}

inline unsigned unfold(const uint8_t* input, Timestamp& x)
{
